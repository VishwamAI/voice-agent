...
async function checkForAskCli() {
    try {
        ...
        const majorVersion = parseInt(stdout[0]);
        const minorVersion = parseInt(stdout.slice(2, 4));
...
function getAskError(method, stderr) {
    ...
    if (errorIndex > -1) {
        ...
        try {
            ...
            return new cli_core_1.JovoCliError({ message: `${method}: ${payload.message}`, module, details });
        }
        catch (error) {
            return new cli_core_1.JovoCliError({ message: `${method}: ${errorString}`, module });
        }
    }
    else {
