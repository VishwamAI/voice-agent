...
export class BuildHook extends AlexaHook {
    install() {
    ...
    validateLocales() {
        for (const locale of this.$context.locales) {
            const resolvedLocales = getResolvedLocales(locale, SupportedLocales, this.$plugin.config.locales);
    ...
    async validateModels() {
        // Validate Jovo model
        const validationTask = new Task(`${OK_HAND} Validating Alexa model files`);
        for (const locale of this.$context.locales) {
            const localeTask = new Task(locale, async () => {
                const model = await this.$cli.project.getModel(locale);
                await this.$cli.project.validateModel(locale, model, JovoModelAlexa.getValidator(model), this.$plugin.name);
                await wait(500);
            });
    ...
    async build() {
        const buildPath = `Path: ./${joinPaths(this.$cli.project.getBuildDirectory(), this.$plugin.platformDirectory)}`;
        const buildTaskTitle = `${STATION} Building Alexa Skill files${printStage(this.$cli.project.stage)}\n${printSubHeadline(buildPath)}`;
        // Define main build task.
        const buildTask = new Task(buildTaskTitle);
        // Update or create Alexa project files, depending on whether it has already been built or not.
        const projectFilesTask = new Task(`Project files`, this.buildProjectFiles.bind(this));
        const interactionModelTask = new Task(`Interaction model`, this.buildInteractionModel.bind(this), {
            enabled: this.$cli.project.config.getParameter('models.enabled') !== false &&
                this.$cli.project.hasModelFiles(this.$context.locales),
        });
        const buildConversationFilesTask = new Task(`Alexa Conversations files`, this.buildConversationsFiles.bind(this), { enabled: this.$context.alexa.isACSkill });
        const buildResponseFilesTask = new Task(`Response files`, this.buildResponseFiles.bind(this), { enabled: this.$context.alexa.isACSkill });
    ...
    async buildReverse() {
        ...
        const reverseBuildTask = new Task(`${REVERSE_ARROWS} Reversing Alexa files`);
        if (this.$cli.project.config.getParameter('models.enabled') !== false) {
            ...
            for (const modelLocale in this.$plugin.config.locales) {
                const resolvedLocales = getResolvedLocales(modelLocale, SupportedLocales, this.$plugin.config.locales);
            ...
            if (this.$cli.project.hasModelFiles(Object.values(buildLocaleMap)) ||
                ...
                if (answer.overwrite === ANSWER_BACKUP) {
                    Log.spacer();
                    // Backup old files.
                    const backupTask = new Task(`${DISK} Creating backups`);
                    ...
                    if (existsSync(this.$cli.project.getModelsDirectory())) {
                        ...
                        const modelTask = new Task(`${this.$cli.project.getModelsDirectory()} -> ${modelsBackupDirectory}`, () => {
                    ...
                    if (existsSync(this.$plugin.resourcesDirectory)) {
                        ...
                        const resourcesTask = new Task(`${this.$plugin.resourcesDirectory} -> ${resourcesBackupDirectory}`, () => {
            ...
            for (const [platformLocale, modelLocale] of Object.entries(buildLocaleMap)) {
                ...
                const localeTask = new Task(`${platformLocale} ${taskDetails}`, async () => {
        ...
        if (this.$context.alexa.isACSkill && ((_a = this.$plugin.config.conversations) === null || _a === void 0 ? void 0 : _a.directory)) {
            if (((_b = this.$plugin.config.conversations) === null || _b === void 0 ? void 0 : _b.acdlDirectory) &&
                ...
                const acdlPath = joinPaths(this.$plugin.config.conversations.directory, this.$plugin.config.conversations.acdlDirectory);
                const copyAcdlFilesTask = new Task(`Copying Alexa Conversations files into ${acdlPath}`, () => copySync(this.$plugin.conversationsDirectory, acdlPath));
            ...
            if (((_c = this.$plugin.config.conversations) === null || _c === void 0 ? void 0 : _c.responsesDirectory) &&
                ...
                const responsesPath = joinPaths(this.$plugin.config.conversations.directory, this.$plugin.config.conversations.responsesDirectory);
                const copyResponseFilesTask = new Task(`Copying Response files into ${responsesPath}`, () => copySync(this.$plugin.responseDirectory, responsesPath));
    ...
    buildProjectFiles() {
        var _a, _b;
        const files = FileBuilder.normalizeFileObject(_get(this.$plugin.config, 'files', {}));
    ...
    checkLocales(projectFiles) {
        ...
        const privacyAndCompliances = _get(projectFiles, `skill-package/["skill.json"].manifest.privacyAndCompliance.locales`, {});
        const publishingInfos = _get(projectFiles, `skill-package/["skill.json"].manifest.publishingInformation.locales`, {});
        for (const locale of locales) {
            const genericLocaleKey = Object.keys(this.$plugin.config.locales || {}).find((key) => [...this.$plugin.config.locales[key]].includes(locale));
    ...
    getDefaultPrivacyAndCompliances() {
        return {
    ...
    async buildLanguageModel(modelLocale, resolvedLocales) {
        const model = _merge(await this.getJovoModel(modelLocale), this.$cli.project.config.getParameter(`models.override.${modelLocale}`));
        try {
    ...
    buildConversationsFiles() {
        const src = joinPaths(this.$plugin.config.conversations.directory, this.$plugin.config.conversations.acdlDirectory);
        if (!existsSync(src)) {
            throw new JovoCliError({
    ...
    buildResponseFiles() {
        const src = joinPaths(this.$plugin.config.conversations.directory, this.$plugin.config.conversations.responsesDirectory);
        if (!existsSync(src)) {
            throw new JovoCliError({
     ...
     */
    getPluginEndpoint() {
        const endpoint = _get(this.$plugin.config, 'endpoint') ||
    ...
    getPlatformModel(locale) {
        const content = readFileSync(this.$plugin.getModelPath(locale), 'utf-8');
     ...
     */
    getPlatformLocales() {
        const files = readdirSync(this.$plugin.modelsPath);
