var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { IsBoolean, IsNotEmpty, IsOfEitherType, IsOptional, IsString, TransformMap, ValidateNested, } from '@jovotech/output';
import { AplGradient } from './AplGradient';
var AplResource = /** @class */ (function () {
    function AplResource() {
    }
    __decorate([
        IsOptional(),
        IsOfEitherType(['string', 'boolean']),
        __metadata("design:type", Object)
    ], AplResource.prototype, "when", void 0);
    __decorate([
        IsOptional(),
        IsBoolean({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "boolean", void 0);
    __decorate([
        IsOptional(),
        IsBoolean({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "booleans", void 0);
    __decorate([
        IsOptional(),
        IsString({ each: true }),
        IsNotEmpty({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "color", void 0);
    __decorate([
        IsOptional(),
        IsString({ each: true }),
        IsNotEmpty({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "colors", void 0);
    __decorate([
        IsOptional(),
        IsString(),
        IsNotEmpty(),
        __metadata("design:type", String)
    ], AplResource.prototype, "description", void 0);
    __decorate([
        IsOptional(),
        IsString({ each: true }),
        IsNotEmpty({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "dimension", void 0);
    __decorate([
        IsOptional(),
        IsString({ each: true }),
        IsNotEmpty({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "dimensions", void 0);
    __decorate([
        IsOptional(),
        IsString({ each: true }),
        IsNotEmpty({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "easing", void 0);
    __decorate([
        IsOptional(),
        IsString({ each: true }),
        IsNotEmpty({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "easings", void 0);
    __decorate([
        IsOptional(),
        ValidateNested({ each: true }),
        TransformMap(function () { return AplGradient; }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "gradient", void 0);
    __decorate([
        TransformMap(function () { return AplGradient; }),
        ValidateNested({ each: true }),
        IsOptional(),
        __metadata("design:type", Object)
    ], AplResource.prototype, "gradients", void 0);
    __decorate([
        IsOptional(),
        IsOfEitherType(['string', 'number'], { each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "number", void 0);
    __decorate([
        IsOptional(),
        IsOfEitherType(['string', 'number'], { each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "numbers", void 0);
    __decorate([
        IsOptional(),
        IsString({ each: true }),
        IsNotEmpty({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "string", void 0);
    __decorate([
        IsOptional(),
        IsString({ each: true }),
        IsNotEmpty({ each: true }),
        __metadata("design:type", Object)
    ], AplResource.prototype, "strings", void 0);
    return AplResource;
}());
export { AplResource };
//# sourceMappingURL=AplResource.js.map