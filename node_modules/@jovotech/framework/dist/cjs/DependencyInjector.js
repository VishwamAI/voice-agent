...
const UnresolvableDependencyError_1 = require("./errors/UnresolvableDependencyError");
const InvalidDependencyError_1 = require("./errors/InvalidDependencyError");
...
class DependencyInjector {
    static resolveInjectionToken(jovo, token, dependencyPath) {
        ...
        if (dependencyPath.includes(token)) {
            throw new CircularDependencyError_1.CircularDependencyError(dependencyPath);
        }
    ...
    static instantiateClassWithTracing(jovo, clazz, dependencyPath, ...predefinedArgs) {
        ...
        for (let argumentIndex = predefinedArgs.length; argumentIndex < argTypes.length; argumentIndex++) {
            ...
            if (!injectionToken) {
                ...
                throw new InvalidDependencyError_1.InvalidDependencyError(clazz, argumentIndex);
            }
            const childNode = DependencyInjector.resolveInjectionToken(jovo, injectionToken, dependencyPath);
            if (!childNode) {
                throw new UnresolvableDependencyError_1.UnresolvableDependencyError(clazz, injectionToken, argumentIndex);
            }
