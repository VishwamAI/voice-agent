var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { IsOptional, IsString, Type, ValidateNested } from '@jovotech/output';
import { CoreResponseUser } from './CoreResponseUser';
import { Session } from './Session';
var Request = /** @class */ (function () {
    function Request() {
    }
    __decorate([
        IsOptional(),
        IsString(),
        __metadata("design:type", String)
    ], Request.prototype, "id", void 0);
    return Request;
}());
export { Request };
var CoreResponseContext = /** @class */ (function () {
    function CoreResponseContext() {
    }
    __decorate([
        ValidateNested(),
        Type(function () { return Request; }),
        __metadata("design:type", Request)
    ], CoreResponseContext.prototype, "request", void 0);
    __decorate([
        ValidateNested(),
        Type(function () { return Session; }),
        __metadata("design:type", Session)
    ], CoreResponseContext.prototype, "session", void 0);
    __decorate([
        ValidateNested(),
        Type(function () { return CoreResponseUser; }),
        __metadata("design:type", CoreResponseUser)
    ], CoreResponseContext.prototype, "user", void 0);
    return CoreResponseContext;
}());
export { CoreResponseContext };
//# sourceMappingURL=CoreResponseContext.js.map