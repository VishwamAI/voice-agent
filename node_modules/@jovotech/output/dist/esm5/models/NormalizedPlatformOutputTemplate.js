var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { IsArray, IsInstance, IsOptional, Type, ValidateNested, } from '..';
import { TransformMessage } from '../decorators/transformation/TransformMessage';
import { IsBooleanOrInstance } from '../decorators/validation/IsBooleanOrInstance';
import { IsStringOrInstance } from '../decorators/validation/IsStringOrInstance';
import { Listen } from './Listen';
import { Card } from './Card';
import { Carousel } from './Carousel';
import { Message } from './Message';
import { QuickReply } from './QuickReply';
var NormalizedPlatformOutputTemplate = /** @class */ (function () {
    function NormalizedPlatformOutputTemplate() {
    }
    __decorate([
        IsOptional(),
        ValidateNested(),
        __metadata("design:type", Object)
    ], NormalizedPlatformOutputTemplate.prototype, "nativeResponse", void 0);
    __decorate([
        IsOptional(),
        IsStringOrInstance(Message),
        TransformMessage(),
        __metadata("design:type", Object)
    ], NormalizedPlatformOutputTemplate.prototype, "message", void 0);
    __decorate([
        IsOptional(),
        IsStringOrInstance(Message),
        Type(function () { return Message; }),
        __metadata("design:type", Object)
    ], NormalizedPlatformOutputTemplate.prototype, "reprompt", void 0);
    __decorate([
        IsOptional(),
        IsBooleanOrInstance(Listen),
        Type(function () { return Listen; }),
        __metadata("design:type", Object)
    ], NormalizedPlatformOutputTemplate.prototype, "listen", void 0);
    __decorate([
        IsOptional(),
        IsArray(),
        IsStringOrInstance(QuickReply, {
            each: true,
        }),
        Type(function () { return QuickReply; }),
        __metadata("design:type", Object)
    ], NormalizedPlatformOutputTemplate.prototype, "quickReplies", void 0);
    __decorate([
        IsOptional(),
        IsInstance(Card),
        ValidateNested(),
        Type(function () { return Card; }),
        __metadata("design:type", Object)
    ], NormalizedPlatformOutputTemplate.prototype, "card", void 0);
    __decorate([
        IsOptional(),
        IsInstance(Carousel),
        ValidateNested(),
        Type(function () { return Carousel; }),
        __metadata("design:type", Object)
    ], NormalizedPlatformOutputTemplate.prototype, "carousel", void 0);
    return NormalizedPlatformOutputTemplate;
}());
export { NormalizedPlatformOutputTemplate };
//# sourceMappingURL=NormalizedPlatformOutputTemplate.js.map